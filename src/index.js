...
const tableModule = (function () {
  ...
  function addToTable(myLibrary) {
    ...
    myLibrary.forEach((element, index) => {
      ...
      newRow.innerHTML = `<td>${element.title}</td><td>${element.author}</td><td>${element.pages}</td><td><button class="p-1 border border-gray-200 rounded-lg my-auto" onclick="toggleRead(event)">${element.read}</button></td><td><button class="p-1 border border-gray-200 rounded-lg my-auto " id="deleteBtn" onclick="tableModule.deleteRow(event)">Delete</button></td>`;
...
const dialogModule = (function () {
  ...
  function addBookToLibrary(event) {
    ...
    tableModule.addToTable(myLibrary);
